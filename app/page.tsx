...
type Role = 'student' | 'parent' | 'educator'
...
function Button({ children, href, variant = 'dark', onClick, className = '' }: { children: React.ReactNode; href?: string; variant?: 'dark' | 'outline' | 'light'; onClick?: () => void; className?: string }) {
  ...
  const props = { onClick, className: `inline-flex min-h-12 items-center justify-center gap-2 rounded-full px-4 sm:px-6 text-xs sm:text-sm font-semibold transition ${styles} ${className}` }
...
export default function Page() {
  ...
  const [pricing, setPricing] = useState<{ plans: Array<{ key: string; label: string; price: number; children: number; tag: string }> } | null>(null)
  ...
  useEffect(() => {
    fetch('/api/waitlist/status')
      ...
      .then((j) => {
        if (j?.data) {
          setLaunched(j.data.launched)
          setLaunchDate(j.data.launchDate)
        }
      })
      .catch(() => {})
  ...
  return <div id="top">
    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      {!launched && (
        <div className="bg-amber-500 text-black py-2.5 px-4 text-center text-xs font-semibold tracking-wide flex justify-center items-center gap-2">
          <span>We are preparing to launch! Join the exclusive waitlist to claim your slots.</span>
          <Link href="/waitlist" className="underline font-bold hover:opacity-85 transition">Join Waitlist &rarr;</Link>
        </div>
      )}
      {launched && launchDate && (
        <div className="bg-green-600 text-white py-2.5 px-4 text-center text-xs font-semibold tracking-wide flex justify-center items-center gap-2">
          <span>🚀 We are officially live! Enrollment is now fully open.</span>
          <Link href="/enroll" className="underline font-bold hover:opacity-85 transition">Enroll Sibling / Learner &rarr;</Link>
        </div>
      )}
...
<div className="relative">
  <div className="relative w-full max-w-[520px] overflow-hidden rounded-[2rem] bg-foreground/85 p-5 text-background backdrop-blur-sm sm:p-7 lg:p-8">
    <div className="absolute right-7 top-7 h-20 w-auto rounded-full border border-background/25 sm:h-24" />

    <div className="absolute bottom-10 left-10 h-28 w-auto rounded-full border border-background/15 sm:h-32" />

    <div className="relative flex min-h-[360px] flex-col justify-between sm:min-h-[400px] lg:min-h-[430px]">
      <div className="flex items-center justify-between gap-4">
        <span className="font-mono text-[9px] uppercase tracking-[.24em] opacity-60 sm:text-[10px]">
          Field notes / 001
        </span>

        <span className="shrink-0 rounded-full border border-background/25 px-3 py-1 font-mono text-[8px] uppercase tracking-widest opacity-70 sm:text-[9px]">
          Est. 2026
        </span>
      </div>

      <div>
        <p className="mb-5 max-w-sm font-serif text-3xl leading-[1.05] sm:text-4xl lg:text-[2.65rem]">
          The future remembers what we choose to carry.
        </p>

        <div className="flex items-center gap-3 border-t border-background/20 pt-4 text-[11px] opacity-65 sm:text-xs">
          <span className="h-2 w-2 shrink-0 rounded-full bg-accent" />
          Learn deeply. Live fully. Pass it on.
        </div>
      </div>
    </div>
  </div>

  <div className="absolute -bottom-14 -left-2 rounded-2xl border border-border bg-card p-3.5 shadow-xl sm:-left-6 sm:p-4">
    <div className="mb-2 font-mono text-[8px] uppercase tracking-widest text-muted-foreground sm:text-[9px]">
      This week's prompt
    </div>

    <p className="max-w-[155px] text-xs font-medium leading-5 sm:max-w-[170px] sm:text-sm">
      What story from home do you want to remember?
    </p>
  </div>
</div>
</div>
...
<section
  id="why"
  className="border-y border-border bg-muted/40 px-5 py-20 sm:px-8 sm:py-28"
>
  <div className="mx-auto max-w-7xl">
    <div className="mb-12 grid gap-6 lg:grid-cols-[.7fr_1.3fr]">
      <p className="font-mono text-[10px] uppercase tracking-[.26em] text-accent">
        01 / Why heritage
      </p>

      <div>
        <h2 className="max-w-3xl text-balance text-4xl font-semibold tracking-[-.035em] sm:text-6xl">
          A stronger sense of self is a superpower.
        </h2>

        <p className="mt-6 max-w-2xl text-base leading-7 text-muted-foreground">
          Not a textbook. Not a one-off workshop. A living, breathing space
          to learn who you are and find your people.
        </p>
      </div>
    </div>
    ...
    <div className="grid gap-px overflow-hidden rounded-3xl border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
      {pillars.map((pillar) => (
        <div
          key={pillar.title}
          className="group bg-background p-6 transition hover:bg-foreground hover:text-background sm:p-7"
        >
...
>
  <div className="mx-auto max-w-7xl">
    <div className="mb-14 flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
      <div>
        <p className="mb-5 font-mono text-[10px] uppercase tracking-[.26em] text-accent">
          02 / The journey
        </p>

        <h2 className="text-4xl font-semibold tracking-[-.035em] sm:text-6xl">
          Learn in a way
          <br />
      ...
      <p className="max-w-xs text-sm leading-6 text-muted-foreground">
        Every part of Heritage Club is designed to move from screen to real
        life.
      </p>
    </div>
    ...
    <div className="grid gap-10 lg:grid-cols-3">
      {journey.map((item, index) => (
        ...
        >
          <div className="mb-16 flex items-center justify-between">
            <span className="font-mono text-[10px] uppercase tracking-[.22em] text-accent">
              {item.label}
            </span>
      ...
      <section
        id="preview"
        className="bg-muted/40 px-5 py-20 sm:px-8 sm:py-28"
      >
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div>
              ...
              <h2 className="max-w-2xl text-4xl font-semibold tracking-[-.035em] sm:text-6xl">
                One club.
                <br />
                <span className="text-muted-foreground">
                  Every perspective.
            ...
            <p className="max-w-xs text-sm leading-6 text-muted-foreground">
              Take a look around. This is a preview of the experience we're
              building together.
            </p>
          </div>
      ...
      >
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr]">
            <div>
              <p className="mb-5 font-mono text-[10px] uppercase tracking-[.26em] text-accent">04 / Membership</p>
              <h2 className="text-4xl font-semibold tracking-[-.035em] sm:text-6xl">A place to<br /><span className="text-muted-foreground">belong and become.</span></h2>
              <p className="mt-6 max-w-sm text-sm leading-6 text-muted-foreground">Select the perfect plan for your family. Every membership unlocks full access to the Heritage Club learning experience.</p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              {pricing?.plans && pricing.plans.length > 0 ? (
                pricing.plans.slice(0, 2).map((p) => (
                  <div key={p.key} onClick={() => setPlan(p.key)} className={`cursor-pointer rounded-3xl border p-6 transition sm:p-8 ${plan === p.key ? 'border-foreground bg-foreground text-background' : 'border-border bg-card hover:border-foreground/40'}`}>
                    <div className="mb-12 flex items-center justify-between">
                      {p.key === 'individual' && <span className="font-mono text-[10px] uppercase tracking-widest opacity-60">Most popular</span>}
                      {plan === p.key && <Check className="h-4 w-4 text-accent" />}
                    </div>
                    <h3 className="text-2xl font-semibold">{p.label}</h3>
                    <p className="mt-2 text-sm leading-6 opacity-65">{p.tag}</p>
                    <div className="mt-8 text-3xl font-semibold">${p.price}<span className="text-sm font-normal opacity-60"> CAD / month</span></div>
                    <div className="mt-7 space-y-3 text-sm">
                      {['Full programme access', 'Live weekend classes', 'Quizzes, assignments & projects', 'Progress tracking', 'Community access'].map(item => <div key={item} className="flex items-center gap-2"><Check className="h-4 w-4 text-accent" />{item}</div>)}
                    </div>
                  </div>
                ))
              ) : (
                <div className="col-span-2 text-center py-8">
                  <p className="text-muted-foreground mb-4">Loading pricing plans...</p>
                  <Link href="/pricing" className="inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground hover:opacity-90">
                    View Pricing Page
                  </Link>
                </div>
              )}
            ...
            <div className="mt-6 text-center">
              <Link href="/pricing" className="inline-flex items-center gap-2 text-sm font-medium text-accent hover:underline">
