...
import { Video, CheckCircle2, Circle, Calendar, Clock, ExternalLink, Lock } from 'lucide-react'
...
interface LessonItem { id: string; title: string; customTitle: string | null; summary: string | null; week: number; xpReward: number; completed: boolean; meetingLink: string | null; recordingLink: string | null; scheduledDate: string | null; scheduledDay: string | null; scheduledTime: string | null }
...
export default function StudentClasses() {
  ...
  const done = unlockedLessons.filter((l) => l.completed).length
  const pct = unlockedLessons.length ? Math.round((done / unlockedLessons.length) * 100) : 0
      ...
      {data && (
        <>
          <Card className="mb-6">
            ...
            <div className="mt-5">
              <div className="mb-2 flex justify-between text-xs text-muted-foreground"><span>Overall completion</span><span>{done}/{unlockedLessons.length}</span></div>
                              ...
                              <div className="flex-1">
                                ...
                                <div className="mt-3 flex flex-wrap gap-2">
                                  ...
                                  {lesson.recordingLink && (
                                    <a href={lesson.recordingLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-medium text-accent hover:underline">
